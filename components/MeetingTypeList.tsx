...
const MeetingTypeList = () => {
  ...
  const createMeeting = async () => {
    ...
    try {
      ...
      if (!call) throw new Error('Không tạo được cuộc họp');
      const startsAt =
      ...
      console.error(error);
      toast({ title: 'Không tạo được cuộc họp' });
    }
  };
  ...
  return (
    <section className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
      ...
      <HomeCard
        ...
        title="Lịch họp"
        description="Lên kế hoạch cho cuộc họp"
        className="bg-purple-1"
        handleClick={() => setMeetingState('isScheduleMeeting')}
      />
      <HomeCard
        img="/icons/recordings.svg"
        title="Xem bản ghi âm"
      ...
      {!callDetail ? (
        ...
        <MeetingModal
          ...
          handleClick={() => {
            ...
            toast({ title: 'Link đã được sao chép' });
          }}
      ...
      <MeetingModal
        ...
        onClose={() => setMeetingState(undefined)}
        title="Bắt đầu một cuộc họp ngay lập tức"
        className="text-center"
